...
class SSSynth_Content(Model):
    ...
    def load_model(self, sess, log_dir):
        ...
        self.encoder_var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_encoder') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_decoder') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_post_net') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'F0_Model') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope = 'Vuv_Model')
    ...
    def get_optimizers(self):
        ...
        self.global_step_con = tf.Variable(0, name='global_step_con', trainable=False)
        self.global_step_vuv = tf.Variable(0, name='global_step_vuv', trainable=False)

        # if config.use_speaker:
        self.content_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stft_encoder') 
        # else:
        ...
        self.con_optimizer = tf.train.AdamOptimizer(learning_rate = config.init_lr)
        ...
        with tf.control_dependencies(update_ops):
            ...
            self.con_train_function = self.con_optimizer.minimize(self.content_loss, global_step=self.global_step_con, var_list=self.content_params)
    ...
    def loss_function(self):
        ...
        self.final_loss = self.recon_loss + config.mu * self.recon_loss_0 + config.lamda * self.content_loss_2
    ...
    def get_summary(self, sess, log_dir):
        ...
        self.content_summary_2 = tf.summary.scalar('content_loss_2', self.content_loss_2)
    ...
    def train(self):
        ...
        self.get_summary(sess, config.log_dir_encoder)
    ...
    def train_model(self,feats_targs, stft_targs, targets_speakers, atbs, sess):
        ...
        _,_,_,_, final_loss, recon_loss, recon_loss_0, content_loss, f0_loss, vuv_loss = sess.run([self.final_train_function, self.con_train_function, self.f0_train_function, self.vuv_train_function, self.final_loss, self.recon_loss, self.recon_loss_0, self.content_loss, self.f0_loss, self.vuv_loss], feed_dict=feed_dict)
    ...
    def model(self):
        ...
        with tf.variable_scope('stft_post_net') as scope: 
            ...
            self.residual_stft = modules.post_net(self.output_stft_1, self.is_train)
