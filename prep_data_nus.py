...
def main():
    ...
    for singer in singers:
        ...
        for lf in sing_wav_files:
            ...
            out_feats, f0 = utils.stft_to_feats(vocals,fs)
            ...
            with h5py.File(config.voice_dir+'nus_'+singer+'_sing_'+lf[:-4]+'.hdf5', mode='w') as hdf5_file:

                hdf5_file.create_dataset("phonemes", strings_p.shape, int)

                hdf5_file["phonemes"][:,] = strings_p

                hdf5_file.create_dataset("voc_stft", voc_stft.shape, np.float32)

                hdf5_file.create_dataset("world_feats", out_feats.shape, np.float32)

                hdf5_file["voc_stft"][:,:] = voc_stft

                hdf5_file["world_feats"][:,:] = out_feats
        ...
        for lf in read_wav_files:
            ...
            with h5py.File(config.voice_dir+'nus_'+singer+'_read_'+lf[:-4]+'.hdf5', mode='w') as hdf5_file:
