...
def train(_):
    if config.mode == "sep":
        ...
        model = models.MaskSep()
    elif config.mode == "chain":
        model = models.Chain()
    model.train()
def train_wav(_):
    model = models.WaveVoc()
    model.train()
def eval_hdf5_file(file_name, speaker_alt = None):
    if config.mode == "voc" or config.mode == "ori":
        model = models.AutoVC()
    elif config.mode == "stft_voc":
        model = models.AutoVCSTFT()
    if config.mode == "voc" or config.mode == "ori" or config.mode == "stft_voc":
